#!/usr/bin/env bash
# This Shell Script will install & configure the following App: MongoDB Community Edition
...
APP_FULL_NAME="MongoDB Community Edition"           # Full name of the App that will be installed (only used for console output)
APP_NAME="mongodb"                                 # The folder name of the App (do not use spaces! only use lowercase and - or _ )
APP_SERVICE="MongoDB"                               # The Name of the Service (Container Name) for the Compose File
...
APP_PORT="27017"                                    # This is the Port Number which will be exposed for public access
TCP_PORT="27017"                                    # This is the Port Number the Docker Container uses internally
SQL_IP="172.30.2.3"                                 # The (fixed) IP for the Docker Container (Gateway must already exist!)
#NPM_IP="172.50.0.29"                                # The (fixed) IP for the nginx-proxy Network
#------------------------------------------------------------------------------------------------------------------------------------------------------
# SSL & AUTH CONFIGURATION
SQL_USER="mdb-admin"                                # The Username for the default admin account ← CHANGE THIS!!
SQL_PASS="e0nCph1k6XXnNV"                           # The Password for the default admin account ← CHANGE THIS!!
#------------------------------------------------------------------------------------------------------------------------------------------------------
# VOLUME & LOCAL MOUNT POINTS
MOUNTPOINT_DATA="${APP_PATH}/dbdata"                # Local Mountpoint
MOUNTPOINT_CONF="${APP_PATH}/config"                # Local Mountpoint
...
CreateLocalMountPoint $MOUNTPOINT_DATA 775 1000
...
services:
  ${APP_NAME}:
    ...
    environment:
      MONGODB_INITDB_ROOT_USERNAME: ${SQL_USER}
      MONGODB_INITDB_ROOT_PASSWORD: ${SQL_PASS}
    
    volumes:
      - ${MOUNTPOINT_DATA}:/data/db
      - ${MOUNTPOINT_CONF}:/data/configdb
