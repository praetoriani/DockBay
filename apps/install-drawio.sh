...
This Shell Script will install & configure the following App: Draw.io
...
APP_FULL_NAME="Draw.io"                             # Full name of the App that will be installed (only used for console output)
APP_NAME="drawio"                                   # The folder name of the App (do not use spaces! only use lowercase and - or _ )
APP_SERVICE="Drawio"                                # The Name of the Service (Container Name) for the Compose File
...
APP_PORT="3102"                                     # This is the Port Number which will be exposed for public access
TCP_PORT="8080"                                     # This is the Port Number the Docker Container uses internally
APP_IP="172.24.0.7"                                 # The (fixed) IP for the Docker Container (Gateway must already exist!)
APP_DOMAIN="drawio.localhost"                       # This is the Domain/URL Traefik is going to use to expose this app
#------------------------------------------------------------------------------------------------------------------------------------------------------
# VOLUME & LOCAL MOUNT POINTS
DATA_VOLUME="DrawioStorage"                         # Name of the Docker Volume -> Only needed if app needs a Docker Volume!
...
services:
  ${APP_SERVICE}:
    ...
    volumes:
      - ${MOUNTPOINT1}:/usr/local/tomcat/webapps/draw/WEB-INF/storage
    environment:
      - DRAWIO_SELF_HOSTED=true
...
# Short Test:
echo "" && curl -s -o /dev/null -w "%{http_code}" http://localhost:3200 && echo ""
# → should return 200
INFO
