...
This Shell Script will install & configure the following App: Faved Bookmark Manager
...
APP_FULL_NAME="Faved Bookmark Manager"              # Full name of the App that will be installed (only used for console output)
APP_NAME="faved"                                    # The folder name of the App (do not use spaces! only use lowercase and - or _ )
APP_SERVICE="Faved"                                 # The Name of the Service (Container Name) for the Compose File
...
APP_PORT="8086"                                     # This is the Port Number which will be exposed for public access
TCP_PORT="80"                                       # This is the Port Number the Docker Container uses internally
APP_SSLPORT="8443"                                  # This is the SSL-Port Number which will be used for SSL/TLS
TCP_SSLPORT="443"                                   # This is the SSL-Port Number which will be used for SSL/TLS
APP_IP="172.24.0.24"                                # The (fixed) IP for the Docker Container (Gateway must already exist!)
APP_DOMAIN="${APP_NAME}.localhost"                  # This is the Domain/URL Traefik is going to use to expose this app
#------------------------------------------------------------------------------------------------------------------------------------------------------
# VOLUME & LOCAL MOUNT POINTS
DOCKERVOL_TMPDATA="${APP_SERVICE}TempData"
DOCKERVOL_STORAGE="${APP_SERVICE}Storage"
DOCKERVOL_APACHE="${APP_SERVICE}ApacheConf"
DOCKERVOL_CRTBOT="${APP_SERVICE}CertBot"
...
# Creating several Docker Volumes Using function from wsl2-lib.sh
CreateNewDockerVolume $DOCKERVOL_TMPDATA $APP_SERVICE
CreateNewDockerVolume $DOCKERVOL_STORAGE $APP_SERVICE
CreateNewDockerVolume $DOCKERVOL_APACHE $APP_SERVICE
CreateNewDockerVolume $DOCKERVOL_CRTBOT $APP_SERVICE
...
services:
  ${APP_NAME}:
    ...
    volumes:
      - ${DOCKERVOL_TMPDATA}:/tmp/
      - ${DOCKERVOL_STORAGE}:/var/www/html/storage/
      - ${DOCKERVOL_APACHE}:/etc/apache2/sites-enabled/
      - ${DOCKERVOL_CRTBOT}:/etc/letsencrypt/
    ports:
      - "${APP_PORT}:${TCP_PORT}"
      - "${APP_SSLPORT}:${TCP_SSLPORT}"
    networks:
      apphost:
        ipv4_address: ${APP_IP}
